...
import numpy as np
import random
from i3dpt_MSA3D_hand import I3D
from i3dpt_MSA3D_hand import Unit3Dpy
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_root', default="/home/chz/IsoGD_hand/train/")
    ...
    random.seed(seed)
    np.random.seed(seed)
    log = open('train_MSA3D_hand.txt','w')
    if mode != 'train':
        epoch_max_number = 1
    ...
    for epoch in range(start_epoch, epoch_max_number):
        if mode == 'train':
            model_name = './{}/MSA3D_hand_{}'.format(save_dir, epoch)
        else:
