...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        self.hand_conv3d_1a_7x7 = conv3d_1a_7x7
        self.hand_maxPool3d_2a_3x3 = MaxPool3dTFPadding(
            kernel_size=(1, 3, 3), stride=(1, 2, 2), padding='SAME')
        # conv conv
        ...
        self.hand_conv3d_2b_1x1 = conv3d_2b_1x1
        ...
        self.maxPool3d_3a_3x3 = MaxPool3dTFPadding(
            kernel_size=(1, 3, 3), stride=(1, 2, 2), padding='SAME')
        self.hand_conv3d_2c_3x3 = conv3d_2c_3x3

        self.hand_maxPool3d_3a_3x3 = MaxPool3dTFPadding(
            kernel_size=(1, 3, 3), stride=(1, 2, 2), padding='SAME')
        ...
        self.hand_mixed_3b = Mixed(192, [64, 96, 128, 16, 32, 32])
        self.hand_mixed_3c = Mixed(256, [128, 128, 192, 32, 96, 64])

        self.hand_maxPool3d_4a_3x3 = MaxPool3dTFPadding(
            kernel_size=(3, 3, 3), stride=(2, 2, 2), padding='SAME')
        ...
        self.hand_mixed_4b = Mixed(480, [192, 96, 208, 16, 48, 64])
        self.hand_mixed_4c = Mixed(512, [160, 112, 224, 24, 64, 64])
        self.hand_mixed_4d = Mixed(512, [128, 128, 256, 24, 64, 64])
        self.hand_mixed_4e = Mixed(512, [112, 144, 288, 32, 64, 64])
        self.hand_mixed_4f = Mixed(528, [256, 160, 320, 32, 128, 128])
        self.nonlocal_4 = NONLocalBlock3D(in_channels=832,sub_sample=False)
        ...
        self.hand_maxPool3d_5a_2x2 = MaxPool3dTFPadding(
            kernel_size=(2, 2, 2), stride=(2, 2, 2), padding='SAME')

        # Mixed 5
        self.hand_mixed_5b = Mixed(832, [256, 160, 320, 32, 128, 128])
        self.hand_mixed_5c = Mixed(832, [384, 192, 384, 48, 128, 128])
        self.sa = da_att_1.PAM_Module(1024)
        self.sc = da_att_1.CAM_Module(1024)
        self.conv51 = torch.nn.Sequential(torch.nn.Conv2d(1024, 1024, 3, padding=1, bias=False),
                                          torch.nn.BatchNorm2d(1024),
                                          torch.nn.ReLU())
        self.conv52 = torch.nn.Sequential(torch.nn.Conv2d(1024, 1024, 3, padding=1, bias=False),
                                          torch.nn.BatchNorm2d(1024),
                                          torch.nn.ReLU())
        self.avg_pool = torch.nn.AvgPool3d((2, 7, 7), (1, 1, 1))
        self.hand_avg_pool = torch.nn.AvgPool3d((2, 7, 7), (1, 1, 1))
        self.conv51 = Unit3Dpy(
            in_channels=1024,
            out_channels=1024,
            kernel_size=(3, 3, 3),
            activation=None,
    ...
    def forward(self, inp,inp_hand):
        # Preprocessing
        
        #print('inp:',inp.shape)
        inp=inp.permute(0,1,4,2,3)
        inp_hand = inp_hand.permute(0, 1, 4, 2, 3)
        out = self.conv3d_1a_7x7(inp+inp_hand)
        #print('conv3d_1a_7x7:',out.shape)
        out = self.maxPool3d_2a_3x3(out)
        #
        # inp_hand = inp_hand.permute(0, 1, 4, 2, 3)
        # out_hand = self.hand_conv3d_1a_7x7(inp_hand)
        # # print('conv3d_1a_7x7:',out.shape)
        # out_hand = self.hand_maxPool3d_2a_3x3(out_hand)

        #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        #print('maxPool3d_2a_3x3:',out.shape)
        out = self.conv3d_2b_1x1(out)
        #print('conv3d_2b_1x1:',out.shape)
        out = self.conv3d_2c_3x3(out)
        #print('conv3d_2c_3x3:',out.shape)
        out = self.maxPool3d_3a_3x3(out)
        #print('maxPool3d_3a_3x3:',out.shape)
        out = self.mixed_3b(out)
        #print('mixed_3b',out.shape)
        out = self.mixed_3c(out)

        # out_hand = self.hand_conv3d_2b_1x1(out_hand)
        # # print('conv3d_2b_1x1:',out.shape)
        # out_hand = self.hand_conv3d_2c_3x3(out_hand)
        # # print('conv3d_2c_3x3:',out.shape)
        # out_hand = self.hand_maxPool3d_3a_3x3(out_hand)
        # # print('maxPool3d_3a_3x3:',out.shape)
        # out_hand = self.hand_mixed_3b(out_hand)
        # # print('mixed_3b',out.shape)
        # out_hand = self.hand_mixed_3c(out_hand)
        # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        # out = out + out_hand
        #print('mixed_3c:',out.shape)
        # out = self.nonlocal_3(out)
        #print('non_local:',out.shape)
        out = self.maxPool3d_4a_3x3(out)
        #print('maxPool3d_4a_3x3:',out.shape)

        out = self.mixed_4b(out)
        #print('mixed_4b:',out.shape)
        out = self.mixed_4c(out)
        #print('mixed_4c:',out.shape)
        out = self.mixed_4d(out)
        #print('mixed_4d:',out.shape)
        out = self.mixed_4e(out)
        #print('mixed_4e:',out.shape)
        out = self.mixed_4f(out)

        # out_hand = self.hand_maxPool3d_4a_3x3(out_hand)
        # out_hand = self.hand_mixed_4b(out_hand)
        # # print('mixed_4b:',out.shape)
        # out_hand = self.hand_mixed_4c(out_hand)
        # # print('mixed_4c:',out.shape)
        # out_hand = self.hand_mixed_4d(out_hand)
        # # print('mixed_4d:',out.shape)
        # out_hand = self.hand_mixed_4e(out_hand)
        # # print('mixed_4e:',out.shape)
        # out_hand = self.hand_mixed_4f(out_hand)
        # print(out_hand.shape)
        # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        #print('mixed_4f:',out.shape)
        # out = self.nonlocal_4(out)
        out = self.maxPool3d_5a_2x2(out)
        #print('.maxPool3d_5a_2x2:',out.shape)
        out = self.mixed_5b(out)
        #print('mixed_5b',out.shape)
        ...
        #
        # out = sa_conv + sc_conv
        #print('mixed_5c:',out.shape)
        out = self.avg_pool(out)
        #print('avg_pool:',out.shape)
        # out = self.dropout(out)
        #
        # out_hand = self.hand_maxPool3d_5a_2x2(out_hand)
        # # print('.maxPool3d_5a_2x2:',out.shape)
        # out_hand = self.hand_mixed_5b(out_hand)
        # # print('mixed_5b',out.shape)
        # out_hand = self.hand_mixed_5c(out_hand)
        # # print('mixed_5c:',out.shape)
        # out_hand = self.hand_avg_pool(out_hand)
        # out = self.conv3d_6a_1x1(out)
        # out_hand = self.conv3d_6b_1x1(out_hand)
        # out = torch.cat((out,out_hand),1)
        # out = self.conv3d_6c_1x1(out)
        # out = self.conv3d_6d_1x1(out)
        # print('avg_pool:',out.shape)
        # out_hand = self.dropout(out_hand)
        # out = out+self.gamma*out_hand
        # print(self.gamma)
        out = self.dropout(out)
        # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        #print('dropout:',out.shape)
        out = self.conv3d_0c_1x1(out)
        #print('conv3d_0c_1x1:',out.shape)
        out = out.squeeze(3)
        #print('squeeze3',out.shape)
        out = out.squeeze(3)
        #print('squeeze3:',out.shape)
        out = out.mean(2)
