...
if __name__ == '__main__':
    ...
    parser.add_argument('-g', '--gpu', default="3", help="gpu")
    parser.add_argument('--i3d_weights_path', type=str, default='./model/model_rgb.pth',
    ...
    random.seed(seed)
    np.random.seed(seed)
    log = open('train_MSA3D_depth.txt','w')
    if mode != 'train':
        epoch_max_number = 1
    ...
    if mode == 'train':
        ...
        test_dataset = IsoGD_Dataset.IsoGD_Dataset(args.test_data_root,
                                                     args.test_round_truth, typ, sn=sn, phase='test')
    ...
    for epoch in range(start_epoch, epoch_max_number):
        if mode == 'train':
            model_name = './{}/MSA3D_depth_{}'.format(save_dir, epoch)
        else:
