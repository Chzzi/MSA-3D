...
import numpy as np
import random
from i3dpt_MSA3D_depth import I3D
from i3dpt_MSA3D_depth import Unit3Dpy
...
parser.add_argument('--i3d_weights_path', type=str, default='./i3d_pretrained_model/model_rgb.pth',
                    help='Path to rgb model state_dict')
parser.add_argument('--test_data_root', default="/home/chz/IsoGD_hand/test/")
parser.add_argument('--test_ground_truth', default="/home/chz/IsoGD_hand/test.txt")
parser.add_argument('--feature_save_path', default="./feature_MSA3D_hand.pkl")
parser.add_argument('--model_path', default="./MSA3D_hand.model")
...
if mode == 'test':
    test_dataset = IsoGD_Dataset.IsoGD_Dataset(args.test_data_root,
                                                 args.test_ground_truth, typ, sn=sn,
                                                 phase='test')
...
def test(dataloader, epoch):
    ...
    with torch.no_grad():
        ...
        for i, (data) in enumerate(dataloader):
            ...
            d = data['data']
            l = data['label']
            outputs, out_logits = model(d.cuda())
            output_features.append(outputs.squeeze(0).cpu().numpy())
            pred = torch.argmax(outputs, dim=1)
            correct += (pred == l.cuda()).sum().item()
            max_num += len(d)
            acc = float(correct) / max_num
            print(' Testing [%2d/%2d, %4d/%4d], Acc: %.4f \t time: %.3f' % (
                epoch, epoch_max_number, i + 1, len(dataloader.dataset) / testing_batch_size, acc,
                time.time() - t))
