...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        self.conv52 = Unit3Dpy(
            ...
            kernel_size=(3, 3, 3),
            padding='SAME')
        # self.avg_pool_att = torch.nn.AvgPool3d((2, 1, 1), (1, 1, 1))
        self.avg_pool = torch.nn.AvgPool3d((2, 7, 7), (1, 1, 1))
        # self.avg_pool_2d = torch.nn.AvgPool2d((7, 7), (1, 1))
    ...
    def forward(self, inp, inp_hand):
        ...
        # output = torch.cat((sa_conv, sc_conv), 1)
        # print(output.shape)

        # print('mixed_5c:',out.shape)
        # out = self.avg_pool_2d(output)
        # out = out.unsqueeze(4)
...
if __name__ == '__main__':
    data = torch.randn((8, 3, 224, 224, 16))
    data = data.permute(0, 1, 4, 2, 3)
    model = I3D(num_classes=400, modality='rgb').cuda()
